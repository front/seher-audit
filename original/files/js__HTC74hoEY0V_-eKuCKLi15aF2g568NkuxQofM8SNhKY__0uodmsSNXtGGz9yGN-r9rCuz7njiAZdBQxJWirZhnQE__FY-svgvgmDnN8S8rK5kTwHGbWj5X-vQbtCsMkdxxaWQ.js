...
(function ($) {
  ...
  Drupal.behaviors.aller_gallery_js = {
    attach: function (context, settings) {
      $('.views-hidden-gallery-thumbs').each(function() {
        var gid = $(this).parent().attr('class');
        var rel = $('.views-hidden-gallery-thumbs li.first a').attr('rel');
        var size = $('.views-hidden-gallery-thumbs li').size();
        var gallery = $('a[rel=' + rel + ']').colorbox();
        // Set the click event on the visible thumbnails
        $('.' + gid + ' .views-visible-gallery-thumbs li').each(function() {
          $(this).click(function(e){
            e.preventDefault();
            gallery.eq($(this).index()).click();
          });
        });
        // Set the click event on the link text
        $('.' + gid + ' .gallery-click-link').click(function(e){
          e.preventDefault();
          gallery.eq(0).click();
        });
        // Modify the link text
        $('.' + gid + '-click-link').text('Se ' + size + ' store bilder');
      });
    }
  };
}(jQuery));
